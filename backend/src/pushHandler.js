...
// ==================================================================
// ✅ Helper: Build PR body (matches the exact GitHub display style)
// ==================================================================
function buildPrBody(owner, repo, analysis) {
  ...
  const issuesLines = issues.length
    ? issues.map((issue) => {
        ...
        const msg = issue?.message ?? 'No description provided';
        const file =
          issue?.file ??
          issue?.filePath ??
          issue?.path ??
        ...
        return `- [${sev}] ${msg} in \`${file}\` (line ${line})`;
      }).join('\n')
...
async function applyFixesAndCreatePR(owner, repo, baseBranch, analysis) {
  try {
    ...
    // ✅ PR body بالنمط المطلوب (مثل الصورة)
    const prBody = buildPrBody(owner, repo, analysis);

    // ✅ إذا فيه PR مفتوح: حدّث الـ body (عشان ما يرجع الشكل القديم)
