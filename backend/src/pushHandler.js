...
// ==================================================================
// ✅ Helper: بناء PR Body بالشكل المطلوب
// ==================================================================
function buildPrBody(owner, repo, analysis) {
  const issues = Array.isArray(analysis?.issues) ? analysis.issues : [];

  const issuesLines = issues.length
    ? issues.map((issue) => {
        ...
        return `- **[${sev}]** ${desc} in \`${file}\` (line ${line})`;
      }).join('\n')
  ...
  return [
    ...
    `- **Original commit:** ${analysis?.commitId || 'N/A'}`,
    `- **Overall security score:** ${analysis?.overallScore ?? 'N/A'} / 100`,
    ``,
    `## Detected issues`,
    ``,
    issuesLines,
...
async function handlePushEvent(payload) {
  // تجاهل أي دفعات (pushes) يقوم بها البوت بنفسه
...
async function processCommitAndApplyFixes(owner, repo, commitId, defaultBranch) {
  ...
  console.log(
    `[Decision] score=${analysis?.overallScore}, threshold=${SECURITY_THRESHOLD}, patches=${analysis?.patches?.length || 0}`
  );
...
async function analyzeSingleCommit(owner, repo, commitId) {
  try {
    ...
    // لو في مشاكل والسكور منخفض بس ما طلع patches… جرّبي توليد patches بشكل منفصل
...
async function applyFixesAndCreatePR(owner, repo, baseBranch, analysis) {
  try {
    ...
    const prBody = buildPrBody(owner, repo, analysis);
    ...
    // ✅ إذا فيه PR مفتوح: حدّثه (عشان ما يرجع الشكل القديم)
    if (existingPRs.length > 0) {
      console.log(`[PR] Existing PR #${existingPRs[0].number} found. Updating title/body...`);
